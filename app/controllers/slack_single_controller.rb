...
class SlackSingleController < ApplicationController
  ...
  def authorize_slack_token # older but simpler
    ...
    if !params.key?(:token)
      render status: 200,
        html: "You are not authorized! Pass token to authorize".html_safe and return
    end
    if params[:token] != slack_token
      render status: 200,
        html: "You are not authorized! Wrong token passed".html_safe and return
    end
  end
  ...
  def authorize_slack_secret
    ...
    if request.headers.key?("X-Slack-Signature")
      render status: 200,
        html: "You are not authorized! Pass secret to authorize".html_safe and return
    end
    if request.headers["X-Slack-Signature"] != slack_secret

      render status: 200,
        html: "You are not authorized! Wrong secret passed".html_safe and return
    end
  end
